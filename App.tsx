...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateBackgroundImage, applyBackgroundToImage } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  }, [currentImage, prompt, editHotspot, addImageToHistory, activeImageIndex]);
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full">
            {activeTab === 'retouch' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-[#ECB984]">
                        {editHotspot ? 'Bagus! Sekarang deskripsikan editan lokal Anda di bawah.' : 'Klik area pada gambar untuk melakukan editan yang presisi.'}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            ...
                            placeholder={editHotspot ? "contoh: 'ubah warna bajuku menjadi biru'" : "Klik dulu sebuah titik pada gambar"}
                            className="flex-grow bg-[#4a2c27] border border-[#ECB984]/20 text-[#FFFEE9] rounded-lg p-5 text-lg focus:ring-2 focus:ring-[#D58258] focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60"
            ...
            {activeTab === 'adjust' && <AdjustmentPanel onApply={handleApplyAdjustment} isLoading={isLoading} isBatchMode={images.length > 1} onGenerateBackground={handleGenerateBackground} onApplyBackground={handleApplyBackground} generatedBackgroundUrl={generatedBackgroundUrl} onUploadBackground={handleBackgroundUpload} />}
            {activeTab === 'filters' && <FilterPanel onApply={handleApplyFilter} isLoading={isLoading} isBatchMode={images.length > 1} />}
        </div>
